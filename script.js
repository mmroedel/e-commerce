...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (productListContainer) {
    fetch("https://fakestoreapi.com/products")
      ...
      .then((produtos) => {
        ...
        setupProductButtons();
        iniciarGraficoProdutos(produtos);
      })
      .catch((error) => {
  ...
  /**
   * Configura os listeners de evento para os botoes 'Adicionar' e 'Comprar'
   */
  ...
  // --- GRAFICO DE PRODUTOS MAIS BEM AVALIADOS ---
  /**
     * Inicia o gráfico de produtos mais bem avaliados.
     * @param {Array} produtos - O array completo de produtos vindo da API.
     */
   function iniciarGraficoProdutos(produtos) {
     const ctx = document.getElementById('myChart');
     if (!ctx) {
       return; 
      }
      const produtosOrdenados = produtos.slice().sort((a, b) => b.rating.rate - a.rating.rate);
      const top5Produtos = produtosOrdenados.slice(0, 5);
      const labelsGrafico = top5Produtos.map(p => 
        p.title.length > 30 ? p.title.substring(0, 30) + '...' : p.title
      );
      const dataGrafico = top5Produtos.map(p => p.rating.rate);
      ...
      new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: 'Top 5 Produtos Mais Bem Avaliados'
            }
          }
        }
      });
      ...
      paymentDetails.innerHTML = `
        ...
        <form class="payment-form">
          ...
          <div class="form-group">
            <label for="numero-cartao">Número do Cartão <i class="card-icon">💳</i></label>
            <input type="tel" id="numero-cartao" placeholder="0000 0000 0000 0000" maxlength="19" required>
          </div>
          ...
          <button type="submit" id="confirmar-pagamento">Confirmar Pagamento R$ ${total.toFixed(2)}</button>
        </form>
          `;
          aplicarMascaras();
        }
